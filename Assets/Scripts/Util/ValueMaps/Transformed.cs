...
namespace ValueMaps {
	public class Transformed2D : ValueMap2D {
		protected readonly ValueMap2D valueMap;
		...
		public Transformed2D(ValueMap2D valueMap, float scale, float angle, float heightMultiplier) :
			this(valueMap, Mathf.Cos(angle) * scale, Mathf.Sin(angle) * scale, -Mathf.Sin(angle) * scale, Mathf.Cos(angle) * scale, heightMultiplier) {}


		public Transformed2D(ValueMap2D valueMap, float x1, float y1, float x2, float y2, float heightMultiplier) {
